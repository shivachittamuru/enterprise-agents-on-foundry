...
## Network exposure in v0.1

The Azure SQL server uses a public endpoint restricted by firewall rules. The
notebooks run on a workstation and connect to the database directly, so a
reachable endpoint is a requirement of the learning workflow rather than an
oversight. Private endpoints and `publicNetworkAccess: 'Disabled'` arrive in the
later security and networking release.

Three parameters control the exposure:

| Parameter | Default | Effect |
|---|---|---|
| `sqlPublicNetworkAccessEnabled` | `true` | Sets `publicNetworkAccess` on the server |
| `allowAzureServices` | `true` | Creates `AllowAllWindowsAzureIps` |
| `developerClientIp` | empty | Creates `AllowDeveloperClient` for one address |

The Azure SQL resource provider refuses any firewall rule write while the public
endpoint is off, returning `DenyPublicEndpointEnabled`. Both rules are therefore
conditional on `sqlPublicNetworkAccessEnabled`, never on their own switch alone.
Setting `enablePrivateNetworking` forces the SQL public endpoint off regardless
of `sqlPublicNetworkAccessEnabled`, so the two flags cannot disagree.

`developerClientIp` accepts a single address, not a range. A rule spanning
`0.0.0.0` to `255.255.255.255` is not expressible through this module. The
`0.0.0.0` to `0.0.0.0` pair used by `AllowAllWindowsAzureIps` is the documented
sentinel for Azure services rather than an address range.

Neither rule grants data access on its own. Microsoft Entra authentication and
database role membership are still required, and SQL authentication is refused
outright.
